...
Keep the workflow the same, but adjust the operator-facing prompt style by language:

- `zh-CN`
  - Use direct, concise Chinese prompts that focus on workspace state, next actions, and the files that need write-back.
- `en`
  - Use concise operational English prompts that state the workspace target, the action, and the expected write-back.
- `ja`
  - Keep Japanese prompts concise and explicit about the target workspace, the action, and the documents that must be updated.
