...
- Git を使っている場合は、既定で各機能完了後に commit と push を行う。毎回 push したくない場合だけ設定で明示的に無効化する
...
- 既定では commit 後に自動 push する
- 毎回 push したくない場合だけ、`auto_push_after_commit = false` を明示的に設定する
- 無効化した場合のみ、push を別アクションとして扱う
